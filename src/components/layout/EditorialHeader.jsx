  ...
  return <>
        ...
        <div className="editorial-container">
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              ...
              return <Link key={to} to={to} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors min-h-0 min-w-0 ${isActive ? 'text-primary bg-primary/5' : 'text-foreground-muted hover:text-foreground hover:bg-background-alt'}`}>
                    {title}
                  </Link>;
            })}
              {navItems.length > 5 && <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                    ...
                    title,
                    to
                  }) => <Link key={to} to={to} className={`block px-4 py-2 text-sm rounded-md transition-colors ${location.pathname === to ? 'text-primary bg-primary/5' : 'text-foreground-muted hover:text-foreground hover:bg-background-alt'}`}>
                          {title}
                        </Link>)}
                    </div>
                  </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Mobile Menu Button */}
              <Button variant="ghost" size="icon" className="lg:hidden min-h-10 min-w-10" onClick={() => setMobileMenuOpen(true)} aria-label="Abrir menu">
      ...
      <AnimatePresence>
        {mobileMenuOpen && <>
          ...
          duration: 0.2
        }} className="fixed inset-0 bg-foreground/20 backdrop-blur-sm z-50 lg:hidden" onClick={() => setMobileMenuOpen(false)} />
          ...
          stiffness: 300
              ...
              <div className="flex flex-col h-full">
                ...
                <nav className="flex-1 overflow-y-auto p-4">
                  ...
                  const isActive = location.pathname === to;
                  return <motion.div key={to} initial={{
                    opacity: 0,
                    x: 20
                  }} animate={{
                    opacity: 1,
                    ...
                    delay: index * 0.05
                          ...
                          <Link to={to} onClick={() => setMobileMenuOpen(false)} className={`flex items-start gap-3 p-4 rounded-lg transition-all ${isActive ? 'bg-primary/10 border border-primary/20' : 'hover:bg-background-alt'}`}>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {description && <span className="block text-sm text-foreground-subtle mt-0.5 line-clamp-2">
                                  {description}
                                </span>}
                            </div>
