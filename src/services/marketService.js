...
export const fetchWhaleTransactions = async (timeframe = '7d') => {
  ...
  try {
    ...
    if (validResponses.length === 0) {
      throw new Error('Não foi possível obter dados de volume de nenhuma exchange. Todas as requisições falharam.');
    }
    ...
    if (transactions.length === 0) {
      throw new Error('Nenhuma movimentação significativa detectada no período. Ajuste o período ou tente novamente mais tarde.');
    }
    ...
    console.error('❌ Erro ao buscar movimentações REAIS de mercado:', error.message);
    throw new Error(`Falha ao obter dados reais de movimentações: ${error.message}`);
  }
};

// REMOVIDO: Função generateMockWhaleTransactions - apenas dados reais agora

// Função para buscar dados on-chain
// NOTA: Dados on-chain requerem integração com APIs blockchain (Etherscan, etc)
// Por enquanto, esta funcionalidade está desabilitada até configuração de API
export const fetchOnChainData = async (timeframe = '7d') => {
  throw new Error('Dados on-chain requerem configuração de API blockchain (Etherscan, etc). Funcionalidade temporariamente desabilitada.');
};

// REMOVIDO: Função generateMockOnChainData - apenas dados reais agora
...
// ✅ TODAS AS FUNÇÕES AGORA USAM APENAS DADOS REAIS DE APIs
// ❌ Dados simulados/mock foram completamente removidos
// Se uma API falhar, o erro será propagado e exibido ao usuário
